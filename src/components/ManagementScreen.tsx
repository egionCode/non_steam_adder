...
const ManagementScreen: Component<ManagementScreenProps> = (props) => {
  ...
  const [currentSgdbId, setCurrentSgdbId] = createSignal<number | null>(
    props.sgdbId,
  );
  ...
  const performSearch = async () => {
    ...
    if (title) {
      ...
      try {
        const games = await invoke<any[]>("search_sgdb_games", {
          query: title,
        });
  ...
  createEffect(() => {
    ...
    if (id) {
      ...
      invoke<SGDBArtwork[]>("get_sgdb_artworks", {
        gameId: id,
        artType: "grids",
        dimensions: "600x900,342x482,660x930,460x215,920x430",
        ...
        .then((grids) => {
          if (!grids) return;
          console.log(`Fetched ${grids.length} grids`);
          
          // Filter for Vertical (Portrait)
          ...
          // Filter for Wide (Landscape)
          const wideArt = grids.filter(g => 
            (g.width && g.height && g.width > g.height) || 
            ["460x215", "920x430", "horizontal"].some(d => g.url.includes(d))
          );
          setWide(wideArt[0] || (grids.length > 1 ? grids[1] : grids[0]) || null);
        })
        .catch(err => console.error("Grids error:", err))
      ...
      invoke<SGDBArtwork[]>("get_sgdb_artworks", {
        ...
        .then((heroes) => {
          console.log(`Fetched ${heroes?.length || 0} heroes`);
          setHero(heroes?.[0] || null);
        })
        .catch(err => console.error("Heroes error:", err))
      ...
      invoke<SGDBArtwork[]>("get_sgdb_artworks", {
        ...
        .then((logos) => {
          console.log(`Fetched ${logos?.length || 0} logos`);
          setLogo(logos?.[0] || null);
        })
        .catch(err => console.error("Logos error:", err))
      ...
      invoke<SGDBArtwork[]>("get_sgdb_artworks", {
        ...
        .then((icons) => {
          console.log(`Fetched ${icons?.length || 0} icons`);
          setIcon(icons?.[0] || null);
        })
          ...
          <div class="flex flex-col items-center justify-center text-center py-20 flex-1 animate-in zoom-in slide-in-from-bottom-12 duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)]">
            <div class="relative mb-12 group">
              ...
              <div class="w-32 h-32 rounded-3xl bg-gradient-to-br from-green-400/20 to-green-600/20 flex items-center justify-center relative z-10 border border-green-400/30 shadow-[0_0_50px_rgba(34,197,94,0.2)] transform rotate-12 group-hover:rotate-0 transition-transform duration-700">
                <div class="text-7xl text-green-400 drop-shadow-[0_0_20px_rgba(34,197,94,0.5)] animate-in zoom-in duration-500 delay-300 fill-mode-backwards">
                  ✓
                </div>
              </div>
            ...
            <div class="animate-steam-slide-up animate-stagger-1 fill-mode-backwards">
              <h2 class="text-4xl font-black mb-3 text-white uppercase tracking-tight">
                Game Added!
              </h2>
              <p class="text-steam-text-muted text-lg max-w-md mb-10 leading-relaxed">
                <strong class="text-white">{searchTitle()}</strong> is ready in
                your library.
                <span class="block mt-2 text-xs opacity-50 uppercase tracking-[0.2em] font-bold text-steam-blue animate-steam-pulse">
        ...
        <div class="relative w-full">
          <Show when={isSyncing()}>
            <div class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-steam-bg-dark/60 backdrop-blur-xl rounded-2xl border border-steam-blue/30 animate-steam-fade-in overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)]">
              ...
              <span class="text-steam-blue font-black uppercase tracking-[0.4em] text-xs animate-steam-pulse drop-shadow-[0_0_10px_rgba(102,192,244,0.5)]">
                Injecting Assets...
              </span>
          ...
          <div
            class="management-grid w-full transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)]"
            style={{
          ...
          >
            ...
            >
              ...
              <div class="flex gap-2 w-full max-w-xl mb-2 group">
                ...
                <button
                  ...
                  class="bg-steam-blue hover:brightness-110 active:scale-[0.97] text-white px-6 py-3 rounded-xl font-black transition-all shadow-xl disabled:opacity-50 flex items-center justify-center min-w-[100px] text-sm uppercase tracking-wider"
                >
              ...
              <div class="inline-block px-3 py-1.5 rounded bg-black/30 border border-white/5 text-[9px] font-mono text-steam-blue/50 break-all max-w-lg mx-auto">
                <span class="text-white/10 mr-2 font-bold uppercase">EXE:</span>
                {props.filePath}
              </div>
            </div>
            ...
            <div
              ...
              class="animate-steam-slide-up animate-stagger-1 fill-mode-backwards"
            >
              <ArtSlot
            ...
            <div
              style={{ "grid-area": "vertical" }}
              class="animate-steam-slide-up animate-stagger-2 fill-mode-backwards h-full"
            >
              <ArtSlot
                label="Library Cover"
                art={grid()}
            ...
            <div
              style={{ "grid-area": "wide" }}
              class="animate-steam-slide-up animate-stagger-3 fill-mode-backwards h-full"
            >
              <ArtSlot
                label="Wide Capsule"
                art={wide()}
            ...
            <div
              ...
              class="animate-steam-slide-up animate-stagger-4 fill-mode-backwards h-full"
            >
              <ArtSlot
                label="Logo"
                art={logo()}
            ...
            <div
              ...
              class="animate-steam-slide-up animate-stagger-5 fill-mode-backwards h-full"
            >
              <ArtSlot
                label="Sidebar Icon"
                art={icon()}
            ...
            <div
              ...
              class="flex gap-4 w-full mt-2 items-center justify-center"
            >
              <button
              ...
              <button
                ...
                disabled={
                  isAnythingLoading() || isSyncing() || !currentSgdbId()
                }
                onClick={handleSync}
              >
                {isSyncing()
    ...
    >
      <Show
        ...
        fallback={
          <div class="flex flex-col items-center gap-2 relative z-10">
            ...
            <span class="text-[10px] text-steam-text-muted uppercase font-bold animate-pulse">
              Scanning...
            </span>
          </div>
        }
      >
        <Show
          when={props.art}
          fallback={
            <div class="text-steam-text-muted text-xs text-center p-5 opacity-50 relative z-10">
              {props.placeholder}
            </div>
          }
        >
          <img
            src={(props.art?.thumb?.endsWith('.webm') || props.art?.thumb?.endsWith('.mp4')) ? props.art?.url : props.art?.thumb}
            alt={props.label}
      ...
      <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-steam-bg-dark/95 to-transparent pt-8 pb-1.5 px-2 text-center pointer-events-none z-10">
        <span class="text-[9px] font-black text-white uppercase tracking-wider">
          {props.label}
        </span>
      </div>
    </div>
